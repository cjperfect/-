<?php   if(!defined('IN_hbxnseo')) exit("Request Error!");
 ...
 *      $GLOBALS['db_tablepre'];
 *
 * @version        $Id: mysqli.class.php 3 2011年12月29日14:56:41 Feng $
 * @package        hbxnseoCms.Libraries
 * @copyright      Copyright (c) 2010-2012 hbxnseo.com
 * @link           http://www.hbxnseo.com
 */
...
{	
    ...
    function Open($pconnect=false)
    {
        ...
        else
        {
            ...
            @mysqli_errno($this->linkid) != 0 && $this->DisplayError("hbxnseo Cms错误警告：连接数据库失败，可能数据库密码不对或数据库服务器出错！");
        ...
        if(!$this->linkid)
        {
            $this->DisplayError("hbxnseo Cms错误警告：连接数据库失败，可能数据库密码不对或数据库服务器出错！");
            exit();
        }

        $serverinfo = @mysqli_get_server_info($this->linkid);

        if ($serverinfo > '4.1' && 'utf8') 
        {
            mysqli_query($this->linkid, "SET character_set_connection=utf8,character_set_results=utf8,character_set_client=binary");
        }
